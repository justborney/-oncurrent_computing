...
//-------------------- thread function --------------------
void thread_exec(std::vector<std::vector<int>> &matrix, int range_min, int range_max, int &result) {
	for (size_t i = range_min; i < range_max; ++i) {
		result += min_value(matrix[i]);
	}
}
...
int main() {
	...
	for (size_t i = 0; i < thread_count; ++i) {
		std::thread temp(std::thread(thread_exec, std::ref(matrix), vec_ind_start, vec_ind_end, std::ref(min_vals.at(i))));
